...
public protocol JSONRPCAPI: Codable {
    /// Domain-specific, e.g.: 'eth', 'net', 'personal'...
...
public struct Client<API: JSONRPCAPI> {
    ...
    fileprivate func decoded<T: Decodable>(data: Data?) -> Result<T> {
    ...
    fileprivate func decoded<T: Decodable>(data: Data?) -> BatchResult<T> {
        Swift.Result<[Response<T>],Swift.Error> {
            guard let data = data else {
                // FIXME with a .failure(/* error */)
                fatalError("\(#file): \(#function); \(#line)")
...
extension Client {
     fileprivate func request<R>(_ request: API.Request<R>, callback: @escaping (Result<R>) -> ()) {
    ...
    fileprivate func request<R>(_ request: API.BatchRequest<R>, callback: @escaping (BatchResult<R>) -> ()) {
        let task = session.dataTask(with: encoded(request: request)) { data, response, error in
            guard error == nil else {
                callback(.failure(error!))
                return
...
extension Client {
    public func callAsFunction<R>(request: API.Request<R>) -> Result<R> {
        Web3Kit.waitFor { self.request(request, callback: $0) }
    }

    public func callAsFunction<R>(request: API.Request<R>...) -> BatchResult<R> {
        Web3Kit.waitFor { self.request(request, callback: $0) }
    }
