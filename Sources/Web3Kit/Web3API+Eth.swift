...
extension JSONRPCRequest where API == Eth {
    ...
    public static func block(_ block: BlockParameter, fullResponse: Bool = false, id: Int = 1) -> API.AnyRequest {
        switch block {
        case .hash:
            return API.Request("getBlockByHash", parameters: [block.rawValue, fullResponse], id: id)
        default:
            return API.Request("getBlockByNumber", parameters: [block.rawValue, fullResponse], id: id)
        }
    }
 ...
 extension Collection where Element: JSONRPCRequest, Element.API == Eth {
    public static func getBalances<S: StringProtocol>(addresses: S..., blockNumber: String = "latest") -> Element.API.AnyBatchRequest {
        addresses.enumerated().map {
            Element.API.Request("getBalance", parameters: [$0.element, blockNumber], id: $0.offset)
        }
    }
